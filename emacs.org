...
#+begin_src emacs-lisp
  (use-package general
    ...
    (laat/leader-keys
      ...
      "bf" '(counsel-recentf :which-key "recent files")
      "br" '(counsel-recentf :which-key "recent files")
      "bm" '(bookmark-set :which-key "set bookmark")
      "bg" '(bookmark-jump :which-key "jump to bookmark")
...
#+begin_src emacs-lisp
  (use-package tempel
    ...
    (defun laat/tempel-setup-capf ()
      (setq-local completion-at-point-functions
                  (cons #'tempel-expand
                        completion-at-point-functions)))

    (add-hook 'conf-mode-hook 'laat/tempel-setup-capf)
    (add-hook 'prog-mode-hook 'laat/tempel-setup-capf)
    (add-hook 'text-mode-hook 'laat/tempel-setup-capf))
...
#+begin_src emacs-lisp
  (use-package eglot
    ...
    :preface
    (defun laat/eglot-buffer (command)
      (ignore-errors
        (eglot-code-actions (buffer-end 0) (buffer-end 1) command t)))
    :general
    (laat/local-leader-keys
      "." '(eglot-code-actions :which-key "code actions")
      "r" '(eglot-rename :which-key "rename")
      "0" '(eglot-format :which-key "format"))
    :custom
...
#+begin_src emacs-lisp
  (use-package typescript-ts-mode
    ...
    :defer t
    :preface
    (defun laat/eglot-ts-organize-imports ()
      (interactive)
      (laat/eglot-buffer "source.organizeImports.ts"))
    (defun laat/eglot-ts-add-missing-imports ()
      (interactive)
      (laat/eglot-buffer "source.addMissingImports.ts"))
    (defun laat/eglot-ts-remove-unused-imports ()
      (interactive)
      (laat/eglot-buffer "source.removeUnusedImports.ts"))
    (defun laat/eglot-ts-fix-all ()
      (interactive)
      (laat/eglot-buffer "source.fixall.ts"))
    (defun laat/eglot-ts-sort-imports ()
      (interactive)
      (laat/eglot-buffer "source.sortImports.ts"))
    :general
    (laat/local-leader-keys
      :states 'normal
      :keymaps 'typescript-ts-mode-map
      "o" '(laat/eglot-ts-organize-imports :which-key "organize import")
      "i" '(laat/eglot-ts-add-missing-imports :which-key "add missing imports")
      "u" '(laat/eglot-ts-remove-unused-imports :which-key "remove unused imports"))
    :custom
...
#+begin_src emacs-lisp
  (use-package org-capture
    ...
    (defun laat/grab-spotify-current-song ()
      (let ((song (do-applescript "
  tell application \"Spotify\"
    ...
    return artistName & \" - \" & trackName & \" (\" & albumName & \")\"
        ...
        (replace-regexp-in-string "^\"" "" (replace-regexp-in-string "\"$" "" song t t) t t)))
    :custom
    (org-capture-templates
     `(("s" "Simple" entry (file+headline ,(concat org-directory "notes.org") "Inbox")
       ...
       ("!" "Song" entry (file+headline ,(concat org-directory "notes.org") "Inbox - Songs")
        "\n** %(laat/grab-spotify-current-song)%?\n\n\n\n")
       ("p" "Protocol")
